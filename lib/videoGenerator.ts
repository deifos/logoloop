...
export class VideoGenerator {
  ...
  async generateVideo(options: VideoGenerationOptions): Promise<ArrayBuffer> {
    ...
    try {
      ...
      if (!codecInfo) {
        throw new Error(
          "No supported video encoding codec found in this browser",
        );
      }
      ...
      console.log(
        ...
        codecInfo.codec,
        "with format:",
        codecInfo.format.constructor.name,
      );
      ...
      for (let frame = 0; frame < totalFrames; frame++) {
        ...
        const backgroundIndex =
          Math.floor(frame / framesPerBackground) % backgroundImages.length;
...
export function downloadVideo(
  buffer: ArrayBuffer,
  filename: string = "logoloop-video.mp4",
