...
export default function PreviewCanvas({
  ...
  const getLogoBounds = useCallback(() => {
    if (!containerRef.current || !displayCanvasRef.current || !logoDimensions) return null;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    const newSize = Math.min(500, Math.max(2, Math.round(dragStartRef.current.size * scale)));
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center text-default-500">
        <div className="text-center">
          <svg className="w-16 h-16 mx-auto mb-4 text-default-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div
      ...
      style={{ cursor: resizeMode ? (isDragging ? 'nwse-resize' : 'default') : 'default' }}
    >
      <canvas
  ...
  useEffect(() => {
    const updatePositions = () => {
      ...
      const corners = [
        ...
        { x: offsetX + (logoX + logoWidth) * scaleX, y: offsetY + logoY * scaleY },
        { x: offsetX + logoX * scaleX, y: offsetY + (logoY + logoHeight) * scaleY },
        { x: offsetX + (logoX + logoWidth) * scaleX, y: offsetY + (logoY + logoHeight) * scaleY },
      ];
